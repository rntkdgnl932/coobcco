...
def go_chango(cla, data):
    try:
        ...
        if data == 'village':
            full_path = "c:\\coobcco\\imgs\\chango_1.png"
            ...
            if imgs_ is None or imgs_ == False:
                print("창고그림이 안보여1")
                full_path = "c:\\coobcco\\imgs\\chango_1_1.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(620, 950, 705, 1005, cla, img)
                ...
                go_ = True
        if data == 'chango':
            full_path = "c:\\coobcco\\imgs\\chango_2.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        if data == 'in':
            ...
            for i in range(5):
                full_path = "c:\\coobcco\\imgs\\jangbi_out\\event_item_2.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            for z in range(3):
                ...
                for i in range(len(item_list)):
                    full_path = "c:\\coobcco\\imgs\\jangbi_out\\" + item_list[i] + ".png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            for z in range(1):
                ...
                item_list = item_list_.split("\n")
                for i in range(len(item_list)):
                    full_path = "c:\\coobcco\\imgs\\auction_in\\" + item_list[i] + ".png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(540, 70, 950, 980, cla, img, 0.8)
                    # test
                ...
                item_list = item_list_.split("\n")
                for i in range(len(item_list)):
                    full_path = "c:\\coobcco\\imgs\\jangbi_in\\" + item_list[i] + ".png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(540, 70, 950, 980, cla, img, 0.8)
                    # test
            ...
            if drag_bool == True:
                ...
                for z in range(2):
                    ...
                    for i in range(len(item_list)):
                        ...
                        if imgs_ is None or imgs_ == False:
                            ...
                            # drag_pos(800, 900, 800, 200, cla)
                            time.sleep(0.5)
                full_path = "c:\\coobcco\\imgs\\jangbi_in\\gold.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is None or imgs_ == False:
                    ...
                    click_pos_2(900, 1005, cla)
                    time.sleep(0.5)
                full_path = "c:\\coobcco\\imgs\\jangbi_in\\silver.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(540, 70, 950, 980, cla, img, 0.9)
                # test
        ...
        if data == 'auction':
            ...
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set_(10, 195, 300, 980, cla, img, 0.85)
            # test
            if imgs_ is None or imgs_ == False:
                print(item_list[i] + "(in_이)가 없다...")
            else:
...
def my_jangbi_in():
    try:
        ...
        if os.path.isfile(file_path4) == True:
            # 파일 읽기
            with open(file_path4, "r", encoding='UTF8') as file:
                lines = file.read()
                file.close()
            print("lines", lines)
        else:
            # \nweg\ngoodbell\nmoogi\nbangugoo\njangsingoo\numool\nsegongsuk\njungsoo\nsilver
            data = "weg\ngoodbell\nmoogi\nbangugoo\njangsingoo\numool\nsegongsuk\njungsoo\nsilver\ngagongsuk\ngwangsuk\namafool\nnamoo\ng_more\nyunmasuk"
            with open(file_path4, "w", encoding='UTF8') as file:
                file.write(data)
                file.close()
                with open(file_path4, "r", encoding='UTF8') as file:
                    lines = file.read()
...
def auction(cla):
    try:
        ...
        while isAuctionReady is False:
            ...
            full_path = "c:\\coobcco\\imgs\\clean\\auction.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        if auction_ready == True:
            ...
            while auction_re is False:
                full_path = "c:\\coobcco\\imgs\\auction\\auction_re.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            for i in range(len(datas)):
                ...
                full_path = "c:\\coobcco\\imgs\\auction\\" + item_list[0] + ".png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is None or imgs_ == False:
                    ...
                    while isSell_1 is False:
                        ...
                        full_path = "c:\\coobcco\\imgs\\auction\\auction_x.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        ...
                        buy_1 = False
                        full_path = "c:\\coobcco\\imgs\\auction\\auction_sell_register_1.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        ...
                        full_path = "c:\\coobcco\\imgs\\auction\\auction_sell_register_11.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set_(200, 300, 350, 400, cla, img, 0.8)
                        if imgs_ is not None:
                        ...
                        if buy_1 == True:
                            ...
                            while isSell_2 is False:
                                ...
                                isSell_2_count += 1
                                buy_2 = False
                                full_path = "c:\\coobcco\\imgs\\auction\\auction_sell_register_2.png"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                ...
                                full_path = "c:\\coobcco\\imgs\\auction\\auction_sell_register_22.png"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set_(400, 300, 550, 400, cla, img, 0.8)
                                if imgs_ is not None:
...
def in_village_ready(cla):
    try:
        ...
        full_path2 = "c:\\coobcco\\imgs\\yotoon.png"
        ...
        if imgs_2 is None or imgs_2 == False:
            print("요툰 아님..?")
            full_path3 = "c:\\coobcco\\imgs\\yotoon2.png"
        ...
        # 마나인지 체크 test
        full_path4 = "c:\\coobcco\\imgs\\mana_1.png"
...
def in_village_go_to(cla):
    try:
        ...
        if isworldmap == True:
            ...
            full_path = "c:\\coobcco\\imgs\\mibalgyun.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is None or imgs_ == False:
                ...
                full_path = "c:\\coobcco\\imgs\\mibalgyun_2.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)

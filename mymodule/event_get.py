...
def go_item_open(cla):
    try:
        ...
        if os.path.isfile(file_path7) == True:
            # 파일 읽기
            with open(file_path7, "r", encoding='UTF8') as file:
                lines = file.read().splitlines()
                file.close()
            print("lines", lines)

        for i in range(len(lines)):
            full_path = "c:\\coobcco\\item\\item_open\\" + lines[i] + ".png"
            ...
            if imgs_ is None or imgs_ == False:
                ...
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(560, 100, 950, 960, cla, img)
                if imgs_ is None or imgs_ == False:
                    print("sayong(이)가 없다...")
...
def go_ticket_open(cla):
    try:
        ...
        if os.path.isfile(file_path8) == True:
            # 파일 읽기
            with open(file_path8, "r", encoding='UTF8') as file:
                lines = file.read().splitlines()
                file.close()
            print("lines", lines)

        for i in range(len(lines)):
            full_path = "c:\\coobcco\\item\\ticket_open\\" + lines[i] + ".png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set_(560, 100, 950, 960, cla, img, 0.9)
            if imgs_ is None or imgs_ == False:
                ...
                click_pos_reg(imgs_.x, imgs_.y, cla)
                time.sleep(1)
                full_path = "c:\\coobcco\\item\\ticket_open\\sayong.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(560, 100, 950, 960, cla, img)
                if imgs_ is None or imgs_ == False:
                ...
                while isticketing is False:
                    full_path = "c:\\coobcco\\item\\ticket_open\\x.png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    ...
                    if imgs_ is not None:
                        ...
                        isconfirm = False
                        full_path = "c:\\coobcco\\item\\ticket_open\\confirm_1.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set(290, 940, 480, 1030, cla, img)
                        if imgs_ is not None:
                            isconfirm = True
                        full_path = "c:\\coobcco\\item\\ticket_open\\confirm_2.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set(290, 940, 480, 1030, cla, img)
                        if imgs_ is not None:
                        ...
                        if isconfirm == False:
                            full_path = "c:\\coobcco\\item\\ticket_open\\balobogi.png"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            ...
                            if imgs_ is not None:
                                ...
                                time.sleep(1)
                                full_path = "c:\\coobcco\\item\\ticket_open\\balobogi_2.png"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set(200, 900, 800, 1030, cla, img)
                                if imgs_ is not None:
                                    ...
                                    time.sleep(1)
                                    full_path = "c:\\coobcco\\item\\ticket_open\\balobogi_1.png"
                                    img_array = np.fromfile(full_path, np.uint8)
                                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                    imgs_ = imgs_set(200, 900, 800, 1030, cla, img)
                                    if imgs_ is not None:
    ...
    try:
        ...
        if data != 'seven_four':
            ...
            if data == 'fivetofive':
                ...
                while isZero is False:
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        time.sleep(0.3)
                        full_path = "c:\\coobcco\\item\\seven_four\\eventandbosang.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set_(380, 270, 550, 330, cla, img, 0.8)
                        if imgs_ is not None and imgs_ != False:
                            click_pos_2(480, 320, cla)
                            full_path = "c:\\coobcco\\item\\55\\open_ready.png"  # 오픈대기 파악
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if int(bom_wind) > 300:
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            time.sleep(1)
                            full_path = "c:\\coobcco\\item\\change\\change.png"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if last_x == 0:
                    full_path = "c:\\coobcco\\imgs\\event_complete_2.png"  # '완료' 그림 갯수 파악
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    for list in pyautogui.locateAllOnScreen(img, region=(15 + plus, 300, 800, 600), confidence=0.8):
                        last_x = list.left
            ...
            while isGetSevenFour is False:
                full_path = "c:\\coobcco\\item\\seven_four\\get_new.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                ...
                if imgs_ is None or imgs_ == False:
                    ...
                    time.sleep(0.5)
                    # 실질적인 받기
                    full_path = "c:\\coobcco\\item\\seven_four\\get_complete.png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set(320, 540, 380, 600, cla, img)
                    if imgs_ is None or imgs_ == False:
                        full_path = "c:\\coobcco\\item\\seven_four\\get_complete_2.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def game_event_item_get(cla):
    try:
        ...
        while isitemGet_1 is False:
            full_path = "c:\\coobcco\\imgs\\event_get.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is not None:
                ...
                while isitemGet_2 is False:
                    ...
                    if imgs_ is not None:
                        ...
                        isitemGet_2 = True
                        print("'일차'가 이제 안보여")
                        full_path = "c:\\coobcco\\imgs\\event_x.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def game_event_get(cla):
    try:
        ...
        while isEventEnd is False:
            full_path = "c:\\coobcco\\imgs\\eventandbosang.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is None or imgs_ == False:
                ...
                time.sleep(1)
                full_path = "c:\\coobcco\\imgs\\special_package\\x.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def achieve_get_(cla):
    try:
        ...
        while isachieve_ is False:
            full_path = "c:\\coobcco\\imgs\\achieve\\upjuk.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                while isMenu is False:
                    ...
                    if result_menu == True:
                        isMenu = True
                        full_path = "c:\\coobcco\\imgs\\achieve\\point.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        #                 time.sleep(0.2)
        #                 full_path = "c:\\coobcco\\imgs\\achieve\\achieve_complete_1.png"
        #                 img_array = np.fromfile(full_path, np.uint8)
        #                 img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        #                 time.sleep(0.2)
        #                 full_path = "c:\\coobcco\\imgs\\achieve\\achieve_complete_2.png"
        #                 img_array = np.fromfile(full_path, np.uint8)
        #                 img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        #                         time.sleep(0.1)
        #                         full_path = "c:\\coobcco\\imgs\\achieve\\achieve_complete_ing.png"
        #                         img_array = np.fromfile(full_path, np.uint8)
        #                         img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def special_package(cla):
    try:
        ...
        #         full_path = "c:\\coobcco\\imgs\\special_package\\point_1.png"
        #         img_array = np.fromfile(full_path, np.uint8)
        #         img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        #         imgs_ = imgs_set_(730, 25, 780, 60, cla, img, 0.8)
        #         if imgs_ is None or imgs_ == False:
        #             print("(point_1)가 없다...")
        #             full_path = "c:\\coobcco\\imgs\\special_package\\point_2.png"
        #             img_array = np.fromfile(full_path, np.uint8)
        #             img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        #             imgs_ = imgs_set_(730, 25, 780, 60, cla, img, 0.8)
        #             if imgs_ is None or imgs_ == False:
        #                 print("(point_2)가 없다...")
        #                 full_path = "c:\\coobcco\\imgs\\special_package\\point_3.png"
        #                 img_array = np.fromfile(full_path, np.uint8)
        #                 img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        #                 imgs_ = imgs_set_(730, 25, 780, 60, cla, img, 0.8)
        #                 if imgs_ is None or imgs_ == False:
        ...
        # if isSP_ == False:
        #     time.sleep(1)
        #     full_path = "c:\\coobcco\\imgs\\special_package\\special_package.png"
        #     img_array = np.fromfile(full_path, np.uint8)
        #     img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)

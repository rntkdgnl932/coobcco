...
def clean_screen(cla, story):
    try:
        ...
        # X 보이면 클릭(보통 캐시샵)
        full_path = "c:\\coobcco\\imgs\\clean\\click_x.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        full_path = "c:\\coobcco\\imgs\\clean\\party_x.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        full_path = "c:\\coobcco\\imgs\\clean\\unreal_error_2.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        for i in range(len(clean_list)):
            full_path = "c:\\coobcco\\imgs\\clean\\" + clean_list[i] + ".png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        # 키 설정
        full_path = "c:\\coobcco\\imgs\\clean\\key_setting_1.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        full_path = "c:\\coobcco\\imgs\\active.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def bangchi_mode(cla):
    try:
        ...
        full_path2 = "c:\\coobcco\\imgs\\bangchi.png"

...
def go_character_select(cla):
    try:
        ...
        full_path = "c:\\coobcco\\imgs\\char_select.png"
        ...
        if imgs_ is None or imgs_ == False:
            print("캐릭터 선택이 안보여")
            full_path = "c:\\coobcco\\imgs\\char_select2.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(20, 20, 200, 200, cla, img)
            ...
            if imgs_ is None or imgs_ == False:
                print("캐릭터 선택2이 안보여")
                full_path = "c:\\coobcco\\imgs\\char_select4.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(20, 20, 200, 200, cla, img)
        ...
        if data == 'load':
            ...
            while isload is False:
                if os.path.isfile(file_path11) == True:
                    # 파일 읽기
                    isload = True
                    with open(file_path11, "r", encoding='UTF8') as file:
                        line = file.read()
                    ...
                    with open(file_path11, "w", encoding='UTF8') as file:
                        file.write("none")
        # result[0] == number, result[1] == cla_ing_, result[2] == myid, result[3] == mylevel, result[4] == mypower, result[5] == mymoney, result[6] == cla_count, result[7] == mypotion_, result[8] == event_bool
        if data == 'new':
...
def myQuest_play_add(cla, data):
    try:
        ...
        if day_[0] == nowDay:
            ...
            if clear_mission == dunjeon_spl_re and id_cla == start[1] and clalal_ == start[0]:

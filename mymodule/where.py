...
def go_somopoom(cla):
    try:
        ...
        full_path = "c:\\coobcco\\imgs\\somopoom.png"
...
def go_tuto_bool(cla):
    try:
        ...
        if imgs_ is None or imgs_ == False:
            print("event1 안보여")
            full_path = "c:\\coobcco\\imgs\\mybag_1.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(600, 30, 950, 90, cla, img)
            if imgs_ is None or imgs_ == False:
                print("mybag_1 안안보여")
                full_path = "c:\\coobcco\\imgs\\mybag_2.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(600, 30, 950, 90, cla, img)
                if imgs_ is None or imgs_ == False:
                    print("mybag_2 안안보여")
                    full_path = "c:\\coobcco\\imgs\\mybag_3.png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set(600, 30, 950, 90, cla, img)
                    if imgs_ is None or imgs_ == False:
...
def go_worldmap(cla, data):
    try:
        ...
        if data == "world":
            full_path = "c:\\coobcco\\imgs\\worldmap.png"
        elif data == "world_moglog":
            full_path = "c:\\coobcco\\imgs\\worldmap_moglog.png"
        elif data == "maul":
            full_path = "c:\\coobcco\\imgs\\midgrd.png"
        elif data == "world_yotoon":
            full_path = "c:\\coobcco\\imgs\\world_yotoon.png"

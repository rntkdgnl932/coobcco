...
def menuOpenCheck(cla, story):
    try:
        ...
        potioned_ch_ = text_check_get(795, 390, 935, 405, cla)
        potioned_ch = potioned_ch_.split("\n")
        pk1 = " ".join(potioned_ch)
        print("menuOpenCheck", pk1)
        if len(pk1) != 0:
            for list in pk1:
                try:
                    if list == 'p' or list == 'P' or list == 'k' or list == 'K':
                        ismenu_ = True
                except:
                    pass
        return ismenu_
...
def go_to_home(data, cla):
    try:
        ...
        from action import go_level, go_boonhae, go_escape, go_bag
                ...
                if result_go_chango == False:
                    ...
                    while isSomopoom is False:
                        ...
                        if result_somopoom == False:
                            ...
                            if mymoney_ >= 1000000 and mylevel >= 20:
                                # 순록 스테이크
...
def dead_die(cla, story):
    try:
        ...
        full_path = "c:\\coobcco\\imgs\\jangsigan.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set(280, 470, 680, 590, cla, img)
        if imgs_ is None or imgs_ == False:
            full_path = "c:\\coobcco\\imgs\\jangsigan_2.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(200, 300, 800, 800, cla, img)
            if imgs_ is None or imgs_ == False:
                full_path = "c:\\coobcco\\imgs\\jangsigan_3.png"
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set(280, 470, 680, 590, cla, img)
                if imgs_ is None or imgs_ == False:
        ...
        full_path = "c:\\coobcco\\imgs\\kakao_games.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        full_path = "c:\\coobcco\\imgs\\boohwal.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set(490, 900, 670, 970, cla, img)
        if imgs_ is None or imgs_ == False:
            full_path = "c:\\coobcco\\imgs\\boohwal_3.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(490, 900, 670, 970, cla, img)
            if imgs_ is None or imgs_ == False:
            ...
            while isVillage is False:
                ...
                if result == False:
                    ...
                    boohwal_cross_y = 60
                    boo_hwal_ = False
                    full_path = "c:\\coobcco\\imgs\\boohwal_cross_1.png"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set(250, 30, 400, 85, cla, img)
                    if imgs_ is None or imgs_ == False:
                        full_path = "c:\\coobcco\\imgs\\boohwal_cross_2.png"
                        img_array = np.fromfile(full_path, np.uint8)
                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                        imgs_ = imgs_set(250, 30, 400, 85, cla, img)
                        if imgs_ is None or imgs_ == False:
                            full_path = "c:\\coobcco\\imgs\\boohwal_cross_3.png"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set(250, 30, 400, 85, cla, img)
                            if imgs_ is None or imgs_ == False:
                                full_path = "c:\\coobcco\\imgs\\boohwal_cross_4.png"
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set(250, 30, 400, 85, cla, img)
                                if imgs_ is not None:
                    ...
                    if boo_hwal_ == False:
                        ...
                        while isBoggo_ is False:
                            ...
                            full_path = "c:\\coobcco\\imgs\\boohwal_boggoo.png"
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            ...
                            if imgs_ is None or imgs_ == False:
                                ...
                                while isHaegol is False:
                                    full_path = "c:\\coobcco\\imgs\\boohwal_haegol.png"
                                    img_array = np.fromfile(full_path, np.uint8)
                                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                    ...
                                    if imgs_ is None or imgs_ == False:
                                        ...
                                        time.sleep(random_int())
                                        full_path = "c:\\coobcco\\imgs\\boohwal_free.png"
                                        img_array = np.fromfile(full_path, np.uint8)
                                        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
...
def go_auto(cla, number):
    try:
        ...
        full_path = "c:\\coobcco\\imgs\\auto.png"
        img_array = np.fromfile(full_path, np.uint8)
        img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        imgs_ = imgs_set(870, 860, 930, 910, cla, img)
        if imgs_ is None or imgs_ == False:
            print("auto 안보여(go_auto)")
            full_path = "c:\\coobcco\\imgs\\auto_quest.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(870, 860, 930, 910, cla, img)
            if imgs_ is None or imgs_ == False:
                print("auto_quest 안보여(go_auto)")
            else:
                go_ = True
                print("auto_quest 보여(go_auto)")
        else:
            go_ = True
            print("auto 보여(go_auto)", imgs_)
...
def potion_count(cla):
    ...
    try:
        ...
        if result_ == True:
            full_path = "c:\\coobcco\\imgs\\potion_small.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            ...
            full_path = "c:\\coobcco\\imgs\\potion_middle.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(580, 120, 950, 950, cla, img)
            ...
            full_path = "c:\\coobcco\\imgs\\potion_big.png"
            img_array = np.fromfile(full_path, np.uint8)
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            imgs_ = imgs_set(580, 120, 950, 950, cla, img)
